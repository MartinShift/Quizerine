...
namespace DbModels.Repository.Interfaces
{
    public class Repository<TEntity> : IRepository<TEntity> where TEntity : class
    {
        ...
        public virtual async Task<ICollection<TEntity>> FindByConditionAsync(Expression<Func<TEntity, bool>> expression) => await this.Entities.Where(expression).ToListAsync().ConfigureAwait(false);
        public virtual async Task<ICollection<TEntity>> GetAllAsync() => await this.Entities.ToListAsync().ConfigureAwait(false);
        ...
        public virtual async Task<bool> ExistsAsync(Expression<Func<TEntity, bool>> expression) => await this.Entities.FindAsync(expression).ConfigureAwait(false) != null;
